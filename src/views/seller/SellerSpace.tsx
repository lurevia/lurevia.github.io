  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const images = await Promise.all(sourceImages.map(async (url) => (
        url.startsWith("data:") ? (await mediaApi.uploadDataUrl(url)).publicUrl :
